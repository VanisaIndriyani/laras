<?php
$page_title = 'Detail Reservasi Ruangan';
$active_menu = 'ruangan';
...
$id = (int)($_GET['id'] ?? 0);
$r = db()->fetchOne("SELECT rr.*, r.nama_ruangan, r.lantai, r.kapasitas, r.fasilitas as fasilitas_ruangan,
...
$r['nama_ruangan'] = $r['nama_ruangan'] ?? 'Ruangan';
$r['lantai'] = $r['lantai'] ?? '-';
$r['kapasitas'] = $r['kapasitas'] ?? 0;
$r['fasilitas_ruangan'] = $r['fasilitas_ruangan'] ?? null;
$r['pemohon'] = $r['pemohon'] ?? '-';
$r['nip'] = $r['nip'] ?? '-';
$r['unit_kerja'] = $r['unit_kerja'] ?? '-';
$r['hp_pemohon'] = $r['hp_pemohon'] ?? null;

$fasilitas_pendukung = json_decode($r['fasilitas_pendukung'] ?? '[]', true);
?>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <h6 class="card-title">
                    <span style="background:#f5f3ff;color:#6d28d9;font-size:10px;font-weight:700;padding:5px 12px;border-radius:999px;margin-right:10px"><?= $r['kode_reservasi'] ?></span>
                    Form Pengajuan Peminjaman Ruangan Rapat
                </h6>
            ...
            <div class="card-body">
                <div class="form-section">
                    <div class="form-section-title">
                        ...
                        <div class="section-text">
                            ...
                            <small>Data pemohon / penanggung jawab pengguna ruangan</small>
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-5">
                            <small class="d-block" style="color:#64748b;margin-bottom:3px">Nama Lengkap Pemohon</small>
                            <strong style="font-size:12px"><?= $r['pemohon'] ?></strong>
                        </div>
                        <div class="col-md-3">
                            <small class="d-block" style="color:#64748b;margin-bottom:3px">NIP Pemohon</small>
                ...
                <div class="form-section">
                    <div class="form-section-title">
                        ...
                        <div class="section-text">
                            <strong>Pemilihan Ruangan & Detail Acara</strong>
                            <small>Informasi ruangan dan rincian kegiatan rapat</small>
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-12">
                            <div class="room-info">
                                <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                                    <div>
                                        <div class="rn"><?= $r['nama_ruangan'] ?></div>
                                        <small><i class="bi bi-building me-1"></i><?= $r['lantai'] ?></small>
                                    </div>
                                    <div style="text-align:right">
                                        <div class="rn">Kapasitas <?= $r['kapasitas'] ?> Orang</div>
                                        <small>Estimasi peserta: <strong><?= $r['estimasi_peserta'] ?> orang</strong></small>
                                    </div>
                                </div>
                                <hr style="border-color:#c7d2fe;margin:10px 0">
                                <small style="color:#4338ca"><i class="bi bi-star-fill me-1"></i> Fasilitas Standar Ruangan:</small><br>
                                <small style="color:#1e40af"><?= $r['fasilitas_ruangan'] ?: '-' ?></small>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <small class="d-block" style="color:#64748b;margin-bottom:3px">Nama Acara / Judul Kegiatan</small>
                            <strong style="font-size:13px;color:#6d28d9"><i class="bi bi-megaphone-fill me-1"></i><?= $r['nama_acara'] ?></strong>
                        </div>
                        <div class="col-md-12">
                            <small class="d-block" style="color:#64748b;margin-bottom:3px">Deskripsi Keperluan / Agenda Rapat</small>
                            <div style="background:#f8fafc;border-radius:12px;padding:14px;border:1px solid #f1f5f9;font-size:11.5px;line-height:1.7"><?= nl2br(sanitize($r['deskripsi'])) ?></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    <div class="form-section-title">
                        ...
                        <div class="section-text">
                            <strong>Waktu Penggunaan Ruangan <span style="color:#64748b">(Format 24 Jam)</span></strong>
                            <small>Jadwal penggunaan ruangan rapat</small>
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="p-3" style="background:linear-gradient(135deg,#ecfdf5,#d1fae5);border-radius:14px;border:1px solid #6ee7b7">
                                <small style="color:#047857;font-weight:600"><i class="bi bi-play-circle-fill me-1"></i> TANGGAL & JAM MULAI</small>
                                <div class="mt-1" style="font-weight:700;font-size:13px;color:#065f46"><?= format_date($r['tanggal_mulai']) ?></div>
                        ...
                        <div class="col-md-6">
                            <div class="p-3" style="background:linear-gradient(135deg,#fef2f2,#fee2e2);border-radius:14px;border:1px solid #fca5a5">
                                <small style="color:#b91c1c;font-weight:600"><i class="bi bi-stop-circle-fill me-1"></i> TANGGAL & JAM SELESAI</small>
                                <div class="mt-1" style="font-weight:700;font-size:13px;color:#991b1b"><?= format_date($r['tanggal_selesai']) ?></div>
                ...
                <div class="form-section">
                    <div class="form-section-title">
                        ...
                        <div class="section-text">
                            <strong>Fasilitas Sarana Pendukung yang Dibutuhkan</strong>
                            <small>Fasilitas tambahan yang diminta pemohon</small>
                        </div>
                    </div>
                    <?php if (empty($fasilitas_pendukung)): ?>
                        <small style="color:#64748b"><i class="bi bi-dash-circle me-1"></i> Tidak membutuhkan fasilitas pendukung tambahan.</small>
                    <?php else: ?>
                        <div class="row g-2">
                            <?php foreach ($fasilitas_pendukung as $f): ?>
                                <div class="col-md-6">
                                    <div class="d-flex align-items-center gap-2 py-2 px-3" style="background:#faf5ff;border:1px solid #e9d5ff;border-radius:10px">
                                        <i class="bi bi-check-circle-fill" style="color:#7c3aed;font-size:13px"></i>
                                        <span style="font-size:11px;font-weight:600;color:#581c87"><?= $f ?></span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
            ...
            <div class="card-body" style="padding:14px">
                <div class="d-flex flex-column gap-2">
                    <?php if (($r['status'] === 'pending' || $r['status'] === 'ditolak') && ($isAdmin || $r['user_id'] == $user['id'])): ?>
                        ...
                        <a href="form.php?id=<?= $r['id'] ?>" class="btn btn-purple w-100"><i class="bi bi-pencil-fill"></i> Edit Data Reservasi</a>
