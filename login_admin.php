...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nip) || empty($password)) {
        set_flash('error', 'NIP dan Password wajib diisi.');
    ...
    if (!password_verify($password, $user['password'])) {
        set_flash('error', 'Password salah. Silakan coba lagi atau hubungi Bagian Umum.');
        redirect(base_url('login_admin.php'));
    }
    ...
    set_flash('success', 'Login Admin berhasil. Selamat datang, ' . $user['nama_lengkap'] . '!');
    redirect(base_url('dashboard.php'));
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        body {
            background:
                radial-gradient(circle at top right, rgba(229, 35, 43, 0.18) 0%, transparent 50%),
                radial-gradient(circle at bottom left, rgba(11, 28, 72, 0.15) 0%, transparent 50%),
                linear-gradient(180deg, #FEF5F5 0%, #F1F5F9 100%);
            font-size: 14px;
            color: var(--slate-700);
            line-height: 1.55;
        ...
        .login-card {
            ...
            box-shadow:
                0 30px 70px -22px rgba(127, 29, 29, 0.22),
                0 10px 28px -10px rgba(127, 29, 29, 0.12),
                0 0 0 1px rgba(229, 35, 43, 0.05);
            position: relative;
            overflow: hidden;
        ...
        .login-card::before {
            ...
            height: 5px;
            background: linear-gradient(90deg, #7F1D1D 0%, #DC2626 45%, #F59E0B 100%);
        }
        @keyframes cardIn {
        ...
        .login-brand .logo-wrap {
            ...
            justify-content: center;
            padding: 9px;
            box-shadow: 0 10px 24px -8px rgba(127, 29, 29, 0.4);
        }
        ...
        .field {
            position: relative;
            margin-bottom: 16px;
            animation: fieldIn .5s cubic-bezier(.2,.8,.2,1) both;
        }
        .field:nth-of-type(1) { animation-delay: .12s; }
        ...
        .field .toggle-pass:hover { background: var(--slate-100); color: var(--red-700); }
        .field .field-input {
            width: 100%;
        ...
        .field.pw .field-input { padding-right: 56px; }
        ...
        .form-actions {
            ...
            margin: 0 0 24px;
            font-size: 13px;
            animation: fieldIn .5s cubic-bezier(.2,.8,.2,1) .32s both;
        }
        .form-actions .check {
            display: inline-flex;
            align-items: center;
        ...
        .form-actions .link:hover { color: var(--navy-800); }

        /* BUTTON LOGIN ADMIN (MERAH) */
        .btn-login {
            width: 100%;
            padding: 18px 22px;
            background: linear-gradient(135deg, var(--red-900) 0%, var(--red-700) 45%, var(--red-500) 100%);
            color: white;
            border: 0;
            border-radius: 18px;
            font-size: 15.5px;
            ...
            justify-content: center;
            gap: 12px;
            box-shadow: 0 18px 34px -12px rgba(127, 29, 29, 0.55), 0 0 0 1px rgba(255,255,255,0.1) inset;
            transition: all .25s cubic-bezier(.2,.8,.2,1);
            position: relative;
            overflow: hidden;
            animation: fieldIn .5s cubic-bezier(.2,.8,.2,1) .4s both;
        }
        .btn-login::before {
            content: '';
            position: absolute;
        ...
        .btn-login:hover {
            ...
            box-shadow: 0 24px 44px -14px rgba(127, 29, 29, 0.65), 0 0 0 1px rgba(255,255,255,0.12) inset;
        }
        ...
        .switch-cta a:hover { color: var(--navy-800); }
        ...
        .page-footer .seal i { color: var(--red-600); }
        ...
        @media (max-width: 520px) {
            ...
            .login-card {
                ...
                border-radius: 22px;
                box-shadow: 0 20px 50px -18px rgba(127, 29, 29, 0.2);
            }
...
<div class="login-wrapper">
    <div class="login-card">
        ...
        <div class="login-header">
            <h1>Login Admin 🔐</h1>
            <p>Panel kontrol untuk admin & bagian umum</p>
        </div>

        <?php render_flash_alerts(); ?>

        <form method="POST" action="<?= base_url('login_admin.php') ?>" novalidate>
            ...
            <div class="field">
                <div class="input-shell">
                    ...
                    <input
                        ...
                        maxlength="30"
                        autofocus>
                    <label for="inp_nip" class="lbl">NIP Administrator</label>
                </div>
            </div>

            <div class="field pw">
                <div class="input-shell">
                    <i class="bi bi-key-fill ic-prefix"></i>
                    <input
                        type="password"
                        id="inp_pass"
                        name="password"
                        ...
                        required>
                    <button type="button" class="toggle-pass" id="eye" title="Tampilkan password" aria-label="Toggle password visibility">
                        <i class="bi bi-eye-fill"></i>
                    </button>
                    <label for="inp_pass" class="lbl">Password Admin</label>
                </div>
            </div>
...
(function(){
    ...
    const pw = document.getElementById('inp_pass');
    if (eye && pw) {
        eye.addEventListener('click', () => {
            const t = pw.type === 'password' ? 'text' : 'password';
            pw.type = t;
            eye.innerHTML = t === 'password' ? '<i class="bi bi-eye-fill"></i>' : '<i class="bi bi-eye-slash-fill"></i>';
        });
    }
})();
</script>
